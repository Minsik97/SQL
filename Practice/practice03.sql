...
부서명(department_name), 현재업무(job_title)를 사번(employee_id) 오름차순
으로 정렬하세요. 부서가 없는 Kimberely(사번 178)은 표시하지 않습니다.
(106건)
*/
select  em.employee_id,
        em.first_name,
...
order by jo.job_title asc, em.employee_id asc;
...
where em.department_id = de.department_id(+)
and  em.job_id = jo.job_id
order by em.employee_id asc, jo.job_title asc;
...
where re.region_name(+) = con.country_name
order by re.region_name asc, con.country_name desc;
...
select  de.department_id 부서번호,
        ...
        re.region_name 지역이름
from employees em, departments de, locations lo, regions re, countries co
...
on em.department_id = de.department_id, employees em2
where em.manager_id = em2.employee_id;
